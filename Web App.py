...
df = load_data("Data/cleaned_sales_data")
...
if select_display == "Welcome and Introduction":
    ...
    Online sales is a critical revenue stream for any business. And because of the digital nature, each transaction can be recorded,
    and used to infer insights about a business - it's products, processes and customers. \n
    In this project, we will completely explore one organizations online transaction's from December 1st, 2009 to December 12th, 2011.
    """)
    requirements_expander = st.beta_expander("Requirements")
    requirements_expander.markdown("""
    * **Python Libraries** streamlit, pandas, numpy, json, joblib, sklearn, matplotlib, plotly, bokeh_chart
...
if select_display == "Time-Series Analysis":
    ...
    if sales_pred == "Projections":
        ...
        VAR = load_models("VAR-time-series-model-4D-interval")
...
if select_display == "Customer Lifetime Value":
    ...
    col2.write(recommentations[0].title())
    col2.write(recommentations[1].title())
    col2.write(recommentations[2].title())
...
* ** Age (T):** The age of the customer's relationship with the company
    \n
    \n
### Predicted current best customers:

The table below showcases which customers are predicted as the most valuable customers.
    """)
